...
const authCheck = async (req, res, next) => {
  ...
  if (!token) return res.status(401).send({status: 401, payload: "Access token missing"});
  ...
  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, decoded) => {
    if (err || foundUser.userName !== decoded.username) {
      ...
      return res.status(403).send({status: 403, payload: "Access token does not match user"});
    }
